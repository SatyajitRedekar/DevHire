...
function DashboardSeeker() {
  ...
  function badgeStyle(status: string) {
    const map: Record<string, string> = {
      ...
      HIRED: 'bg-emerald-100/80 text-emerald-700 border-emerald-200',
    }
                          ...
                          <tr key={app.id} className="hover:bg-white/50 transition-colors group cursor-pointer" onClick={() => navigate(`/jobs/${app.job_id || app.jobId}`)}>
                            ...
                            <td className="px-6 py-5 text-right">
                              <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold border shadow-sm ${badgeStyle(app.status)}`}>
                                ...
                                {app.status === 'HIRED' && <svg className="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>}
